...
async function createMasterEngine(workerArgs, provider = new Provider()) {
  ...
  provider.define(ClusterManager, [], () => new ClusterManager(workerArgs));
  await masterEngine.mount(ClusterPlugin, { deps: [{ id: ClusterManager }] });

  /** @type {Engine[]} **/
  const [ runtimeEngine ] = await provider.require({ id: Engine });
  const clusterModule = defineReady({}, { id: ClusterManager, method: 'start' });
